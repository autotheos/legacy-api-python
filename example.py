...
current_block = legacy.lgcy.get_current_block()
logger.debug('Current block: ')
logger.debug(json.dumps(current_block, indent=2))
logger.debug('-----------')

previous_block = tron.lgcy.get_block(0)
...
genesis_block_count = tron.lgcy.get_block_transaction_count('earliest')
logger.debug('Genesis Block Transaction Count: ')
logger.debug('Transactions:' + str(genesis_block_count))
logger.debug('-----------')

transaction = tron.lgcy.get_transaction('757a14cef293c69b1cf9b9d3d19c2e40a330c640b05c6ffa4d54609a9628758c')
...
account_info = tron.lgcy.get_account('TKLnCNY5EsLNCvCXQTCn1dtqvc6vHhJUyJ')
...
balance = tron.lgcy.get_account('TKLnCNY5EsLNCvCXQTCn1dtqvc6vHhJUyJ')
...
band_width = tron.lgcy.get_band_width('TKLnCNY5EsLNCvCXQTCn1dtqvc6vHhJUyJ')
...
list_nodes = tron.lgcy.list_nodes()
...
block_ids = tron.lgcy.get_block_range(30, 35)
...
# send = tron.send_lgcy('TGEJj8eus46QMHPgWQe1FJ2ymBXRm96fn1', 10)
# logger.debug('Send LGCY transaction: ')
...
first_transaction = tron.lgcy.get_transaction_from_block(0, 0)
